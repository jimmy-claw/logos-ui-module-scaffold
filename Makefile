...
build-module: setup-nix-merged
	...
	cd $(BUILD_MODULE) && cmake .. $(CMAKE_FLAGS) \
		...
		&& cmake --build . --target your_module_plugin -j$$(nproc)
	@echo "your_module_plugin ready at: $(BUILD_MODULE)/your_module_plugin.so"

## Install your_module to logos-app modules dir
...
build-ui-plugin: setup-nix-merged
	...
	@echo "your_ui plugin ready at: $(BUILD_UI_PLUGIN)/libyour_ui.so"

## Install your_ui plugin to logos-app plugins dir
install: build-ui-plugin
	mkdir -p ~/.local/share/Logos/LogosAppNix/plugins/your_ui
	cp $(BUILD_UI_PLUGIN)/libyour_ui.so ~/.local/share/Logos/LogosAppNix/plugins/your_ui/your_ui.so
	@echo "your_ui installed to ~/.local/share/Logos/LogosAppNix/plugins/your_ui/your_ui.so"

## Install everything: your_ui + your_module
